...
from cmdb.models import PhoneInfo
from cmdb.models import Test

def data_add(request):
    #test1 = Test(name='runoob')
    test1 = PhoneInfo(user = "test123",pwd = "test321")
    test1.save()
...
def data_del(request):
    ...
    # test1 = UserInfo(user = "test123",pwd = "test321")
    # test1.save()
    PhoneInfo.objects.filter(user ="123").delete()
    return HttpResponse("<p>全部数据删除成功！！</p>")
...
def data_update(request):
    ...
    test1 = PhoneInfo.objects.get(user="test")
    test1.pwd = 'test123'
    test1.save()
